...
async function fetchAllStores(cityHint) {
  const baseQuery = `
  ...
  let rows;
  if (cityHint) {
    const r = await pool.query(baseQuery.replace('ORDER BY', `AND LOWER(s.city) LIKE LOWER($1) ORDER BY`), [`%${cityHint}%`]);
    rows = r.rows.length > 0 ? r.rows : (await pool.query(baseQuery)).rows;
  } else {
    rows = (await pool.query(baseQuery)).rows;
  }
...
function formatStores(stores) {
  ...
  return stores.map((s, i) => {
    const svLines = s.services.map(sv =>
...
## DATOS REALES DE NEXO — consulta en vivo a la base de datos

⚠️ REGLA ABSOLUTA: Solo puedes mencionar lo que aparece a continuación. Nunca inventes tiendas, servicios, precios ni horarios.
...
- **Buscar tiendas / ver servicios**: usa directamente los datos inyectados arriba. No necesitas herramientas.
- **Ver disponibilidad**: si está en los datos de arriba, muéstrala. Si el usuario pregunta por una fecha no inyectada, pídele que confirme la fecha y dile que la consultarás.
- **Agendar**: llama \`agendar_cita\` SOLO cuando el usuario confirme explícitamente. Usa los IDs y slots exactos de los datos arriba. El \`startTime\` debe tener formato \`YYYY-MM-DDThh:mm:00-05:00\` (hora Colombia). Ejemplo: slot 12:00 del 16 de mayo → \`2026-05-16T12:00:00-05:00\`.
...
async function processMessage(userId, userMessage, conversationHistory = []) {
  ...
  // Escanear el mensaje actual + los últimos 4 para extraer ciudad y fechas
  const recentText = [
    ...conversationHistory.slice(-4).map(m => (typeof m.content === 'string' ? m.content : '')),
    userMessage,
  ].join(' ');

  const cityHint  = extractCityHint(recentText);
  const dateHints = extractDateHints(recentText, isoHoy);

  // Pre-cargar datos reales de la DB antes de llamar al LLM
  const stores = await fetchAllStores(cityHint);
